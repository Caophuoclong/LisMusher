...
function Room(props) {
    ...
    const handleJoinRoom = async ()=>{
        const roomName = document.getElementById("join-room").value;
        const url = uri + "/dashboard/joinroom";
        const token = JSON.parse(window.localStorage.getItem("token"));
        const headers = {
            authorization: token,
        }
        ...
        const action = addRoom(roomName);
        dispatch(action);
        if(response.status === 400) alert("Khong ton tai ten phong"); 
        document.getElementById("join-room").value = '';
    ...
    return (
        <div className="icon room-container">
            ...
            <CreateAndJoinRoom createRoom={handleCreateRoom} joinRoom={handleJoinRoom}/>
        </div>
    );
}
